...
public class ExceptionControllerAdvice {
	...
	public ResponseEntity<ErrorInfo> MethodArgumentTypeMismatchExceptionHandler(MethodArgumentTypeMismatchException exception) {
		ErrorInfo error = new ErrorInfo();
		error.setErrorMessage("MethodArgumentTypeMismatchException occured:" + exception.getMessage().replace("\"", "").split(";")[1]);
		error.setTimestamp(LocalDateTime.now());
		error.setErrorCode(HttpStatus.CONFLICT.value());
		return new ResponseEntity<ErrorInfo>(error, HttpStatus.CONFLICT);
	}
